...
public interface EstadoMensajeRepository extends JpaRepository<EstadoMensaje, EstadoMensajeId> {
    ...
    /**
     * Estados de entrega/lectura para un conjunto de mensajes.
     */
    List<EstadoMensaje> findByIdMensajeIdIn(Collection<UUID> mensajeIds);

    /**
     * IDs de mensajes no entregados de una conversación para un usuario.
     * Se usa al cargar historial para reflejar "recibido" en el remitente.
     */
    @Query("""
            SELECT e.id.mensajeId FROM EstadoMensaje e
            WHERE e.id.usuarioId = :usuarioId
              AND e.entregadoEn IS NULL
            ...
            """)
    List<UUID> findMensajeIdsPendientesDeEntrega(
            @Param("conversacionId") UUID conversacionId,
            @Param("usuarioId") UUID usuarioId);

    /**
     * Bulk update: marca como entregados todos los mensajes pendientes en una conversación.
     */
    @Modifying
    @Query("""
            UPDATE EstadoMensaje e
            SET e.entregadoEn = :ahora
            WHERE e.id.usuarioId = :usuarioId
              AND e.entregadoEn IS NULL
              AND e.id.mensajeId IN (
                  SELECT m.id FROM Mensaje m
            ...
            """)
    int marcarTodosEntregadosPorConversacion(
            @Param("conversacionId") UUID conversacionId,
            @Param("usuarioId") UUID usuarioId,
    ...
    @Query("""
            UPDATE EstadoMensaje e
            SET e.leidoEn = :ahora,
                e.entregadoEn = COALESCE(e.entregadoEn, :ahora)
            WHERE e.id.usuarioId = :usuarioId
              AND e.leidoEn IS NULL
              AND e.id.mensajeId IN (
                  SELECT m.id FROM Mensaje m
