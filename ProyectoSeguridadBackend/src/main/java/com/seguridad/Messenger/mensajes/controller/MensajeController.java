...
public class MensajeController {
    ...
    @Operation(summary = "Enviar mensaje",
            description = "Envía un mensaje en la conversación. El campo `tipo` determina qué otros campos son obligatorios: " +
                    "**TEXTO** requiere `contenido` legacy o payload E2EE (`cifradoAlg`, `cifradoKeyId`, `cifradoIv`, `contenidoCifrado`); " +
    ...
    public MensajeResponse enviarMensaje(
            ...
            @Parameter(description = "Contenido de texto legacy para TEXTO") @RequestPart(required = false) String contenido,
            @Parameter(description = "Algoritmo E2EE") @RequestPart(required = false) String cifradoAlg,
            @Parameter(description = "KeyId E2EE") @RequestPart(required = false) String cifradoKeyId,
            @Parameter(description = "IV/nonce E2EE en base64") @RequestPart(required = false) String cifradoIv,
            @Parameter(description = "Ciphertext E2EE en base64") @RequestPart(required = false) String contenidoCifrado,
        ...
        return mensajeService.enviarMensaje(
                ...
                contenido,
                buildCifradoPayload(cifradoAlg, cifradoKeyId, cifradoIv, contenidoCifrado),
    ...
    private CifradoPayload buildCifradoPayload(String alg, String keyId, String iv, String ciphertext) {
        if (alg == null && keyId == null && iv == null && ciphertext == null) {
            return null;
        }

        return new CifradoPayload(alg, keyId, iv, ciphertext);
    }
