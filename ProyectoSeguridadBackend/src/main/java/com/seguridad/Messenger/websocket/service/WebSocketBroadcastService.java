...
import com.seguridad.Messenger.websocket.crypto.WsCryptoEnvelope;
import com.seguridad.Messenger.websocket.crypto.WsCryptoService;
...
public class WebSocketBroadcastService {
    ...
    public void enviarAUsuario(UUID usuarioId, WebSocketEvent<?> evento) {
        try {
            ...
            log.warn("Error cifrando/enviando evento WebSocket tipo={} usuarioId={}: {}",
                    evento.tipo(), usuarioId, e.getMessage());
        }
    }

    private WsCryptoEnvelope cifrarEvento(WebSocketEvent<?> evento) throws Exception {
        return wsCryptoService.encryptJson(objectMapper.writeValueAsString(evento));
    }
