...
import com.seguridad.Messenger.auth.dto.ForgotPasswordRequest;
import com.seguridad.Messenger.auth.dto.LoginRequest;
import com.seguridad.Messenger.auth.dto.LoginResponse;
import com.seguridad.Messenger.auth.dto.ResetPasswordRequest;
...
import com.seguridad.Messenger.auth.service.PasswordResetService;
...
public class AuthController {
    ...
    @Operation(summary = "Solicitar enlace de recuperación de contraseña",
            description = "Si el correo está registrado y la cuenta activa, envía un email con un enlace " +
                    "de reseteo válido por un tiempo limitado. La respuesta es siempre la misma para evitar " +
                    "enumeración de usuarios.")
    @ApiResponse(responseCode = "200", description = "Respuesta genérica de confirmación")
    @ApiResponse(responseCode = "400", description = "Email inválido",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    public ResponseEntity<RespuestaGenericaResponse> forgotPassword(
            @Valid @RequestBody ForgotPasswordRequest request) {
        return ResponseEntity.ok(passwordResetService.solicitarReset(request.email()));
    }

    @PostMapping("/reset-password")
    @Operation(summary = "Confirmar nuevo password con el token recibido por correo",
            description = "Valida el token, actualiza la contraseña encriptada con BCrypt, marca el token " +
                    "como usado e invalida sesiones existentes del usuario.")
    @ApiResponse(responseCode = "200", description = "Contraseña actualizada")
    @ApiResponse(responseCode = "400", description = "Token inválido, expirado o ya usado",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    public ResponseEntity<RespuestaGenericaResponse> resetPassword(
            @Valid @RequestBody ResetPasswordRequest request) {
        return ResponseEntity.ok(
                passwordResetService.resetearPassword(request.token(), request.newPassword()));
    }
