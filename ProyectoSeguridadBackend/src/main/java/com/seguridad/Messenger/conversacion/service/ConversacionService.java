...
public class ConversacionService {
    ...
    private ChatResumenResponse toChatResumenResponse(Conversacion c, UUID usuarioId) {
        ...
        return new ChatResumenResponse(
                ...
                ultimoMensajeDto, contactoId, noLeidos);
    }

    private String generarPreview(Mensaje m) {
        if (m.isEliminadoParaTodos()) return "Mensaje eliminado";
        if (m.getContenidoCifrado() != null && !m.getContenidoCifrado().isBlank()) return "Mensaje cifrado";
        return switch (m.getTipo()) {
            case TEXTO     -> truncar(dbCryptoService.decrypt(m.getContenido()), 60);
            case IMAGEN    -> "Foto";
            case VIDEO     -> "Video";
            case AUDIO     -> "Audio";
