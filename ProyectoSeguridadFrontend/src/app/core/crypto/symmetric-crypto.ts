export type SymmetricCryptoAlgorithm = 'AES_GCM' | 'DES_CBC' | 'CHACHA20';
...
interface SymmetricCipherContext {
  keyBytes: Uint8Array;
  keyId: string;
}
...
export function normalizeSymmetricAlgorithm(
  algorithm: string | null | undefined,
): SymmetricCryptoAlgorithm {
  const normalized = (algorithm || 'AES_GCM').trim().toUpperCase();
  ...
  throw new Error(`Algoritmo de cifrado no soportado: ${algorithm}`);
}

export async function encryptSymmetricString(
  plainText: string,
  secret: string,
  algorithmValue: string | null | undefined,
): Promise<SymmetricEncryptedPayload> {
  const algorithm = normalizeSymmetricAlgorithm(algorithmValue);
  const context = await buildContext(secret);
  const encrypted = await encryptWithAlgorithm(plainText, context.keyBytes, algorithm);
...
export async function decryptSymmetricString(
  payload: Pick<SymmetricEncryptedPayload, 'alg' | 'iv' | 'ciphertext'>,
  secret: string,
): Promise<string> {
  const algorithm = normalizeSymmetricAlgorithm(payload.alg);
  const context = await buildContext(secret);

  switch (algorithm) {
    case 'AES_GCM':
      return decryptAesGcm(payload, context.keyBytes);
    case 'DES_CBC':
      return decryptDesCbc(payload, context.keyBytes);
    case 'CHACHA20':
      return decryptChaCha20(payload, context.keyBytes);
  }
}

async function buildContext(secret: string): Promise<SymmetricCipherContext> {
...
): Promise<Pick<SymmetricEncryptedPayload, 'iv' | 'ciphertext'>> {
  switch (algorithm) {
    case 'AES_GCM':
      return encryptAesGcm(plainText, keyBytes);
    case 'DES_CBC':
      return encryptDesCbc(plainText, keyBytes);
    case 'CHACHA20':
      return encryptChaCha20(plainText, keyBytes);
  }
}
...
async function encryptAesGcm(
  ...
  const cryptoKey = await crypto.subtle.importKey('raw', toArrayBuffer(keyBytes), 'AES-GCM', false, [
    'encrypt',
  ]);
...
async function decryptAesGcm(
  payload: Pick<SymmetricEncryptedPayload, 'iv' | 'ciphertext'>,
  keyBytes: Uint8Array,
): Promise<string> {
  const cryptoKey = await crypto.subtle.importKey('raw', toArrayBuffer(keyBytes), 'AES-GCM', false, [
    'decrypt',
...
async function encryptDesCbc(
  plainText: string,
  keyBytes: Uint8Array,
): Promise<Pick<SymmetricEncryptedPayload, 'iv' | 'ciphertext'>> {
  const iv = randomBytes(8);
  const encrypted = desCbcEncrypt(encoder.encode(plainText), keyBytes.slice(0, 8), iv);
...
async function decryptDesCbc(
  payload: Pick<SymmetricEncryptedPayload, 'iv' | 'ciphertext'>,
  keyBytes: Uint8Array,
): Promise<string> {
  const decrypted = desCbcDecrypt(base64ToBytes(payload.ciphertext), keyBytes.slice(0, 8), base64ToBytes(payload.iv));
  return decoder.decode(decrypted);
}
...
async function encryptChaCha20(
  ...
  const encrypted = chacha20Xor(encoder.encode(plainText), keyBytes, nonce);
...
async function decryptChaCha20(
  ...
  const decrypted = chacha20Xor(base64ToBytes(payload.ciphertext), keyBytes, base64ToBytes(payload.iv));
  return decoder.decode(decrypted);
}
